...
async function parseJson(res: Response, label: string) {
  ...
  if (!ct.includes('application/json')) {
    throw new Error(`${label}: attendu JSON, reçu ${ct || 'inconnu'} (status ${res.status}) — ${text.slice(0, 150)}`);
  }
  try {
...
async function getToken(): Promise<string> {
  ...
  try {
    ...
    throw new Error(`Impossible de joindre Umami à ${url}: ${e instanceof Error ? e.message : e}`);
  }

  const data = await parseJson(res, `Login ${url}`);

  if (!res.ok || !data.token) {
    throw new Error(`Login échoué (${res.status}): ${JSON.stringify(data).slice(0, 200)}`);
  }
...
async function umamiGet(path: string, token: string) {
  ...
  try {
    ...
    throw new Error(`Fetch ${path} échoué: ${e instanceof Error ? e.message : e}`);
  }
...
export async function GET(req: NextRequest) {
  // Middleware already enforced super-admin auth for /api/admin/*
  ...
  try {
    ...
    logger.error({ err: message }, 'Umami API error');
