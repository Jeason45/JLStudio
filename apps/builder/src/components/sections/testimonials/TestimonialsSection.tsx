...
function PulseFeatured({ content, items, sectionId }: { content: Partial<TestimonialsContent>; items: TestimonialItem[]; accent: string; styleOverrides?: StyleOverrides; sectionId: string }) {
  ...
  const defaultQuotes = [
    "Il a transformé notre festival en une expérience inoubliable. La montée en puissance de son set de clôture a laissé 10 000 personnes sans voix.",
    "Trois années consécutives qu'il assure nos soirées corporate. Professionnel, à l'écoute et toujours dans le bon tempo pour nos événements.",
    "Le meilleur DJ résident qu'on ait eu. Il lit la foule comme personne et sait exactement quand lâcher le drop.",
  ]

  const defaultAuthors = [
    { name: 'Marc D.', role: 'Directeur Artistique Festival' },
    { name: 'Sophie L.', role: 'Event Manager' },
    { name: 'Antoine R.', role: 'Gérant de Club' },
  ]
  ...
  const defaultAvatars = [
    ...
    'https://images.unsplash.com/photo-1571266028243-d220c6a6dee1?w=900&q=85',
  ]
  ...
  const avatars = items.length > 0
    ? items.map((item, i) => item.avatar || defaultAvatars[i % defaultAvatars.length])
    : defaultAvatars
  ...
  return (
    <section {...elementProps(sectionId, 'wrapper', 'container', 'Testimonials Section')} style={{ backgroundColor: '#0D0D0D', color: '#FFFFFF', fontFamily: "'GeneralSans Variable', 'General Sans', sans-serif" }}>
      <style>{`
        @media (max-width: 768px) {
          .pulse-testimonial-split { flex-direction: column !important; }
          .pulse-testimonial-split > div { flex: 1 1 100% !important; min-height: 50vh; }
        }
      `}</style>
      <div {...elementProps(sectionId, 'splitLayout', 'container', 'Split Layout')} className="pulse-testimonial-split flex flex-row w-full" style={{ minHeight: '620px' }}>
        {/* LEFT — Quote (50%) */}
        <div {...elementProps(sectionId, 'contentPanel', 'container', 'Content Panel')} className="flex flex-col" style={{ flex: '1 1 50%', backgroundColor: '#0A0A0A', color: '#FFFFFF', padding: '60px' }}>
          {/* Top label */}
          <p
            {...elementProps(sectionId, 'eyebrow', 'text')}
            style={{ fontSize: '14px', fontWeight: 500, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#00E5FF', marginBottom: 'auto' }}
          >
            {content.eyebrow || content.title || 'Témoignages'}
          </p>
          ...
          <div className="flex flex-col" style={{ maxWidth: '536px', marginTop: 'auto', marginLeft: 'auto' }}>
            ...
            <div style={{ marginTop: 'auto', marginBottom: '40px', marginLeft: 'auto' }}>
              {quotes.map((quote, i) => (
                <div key={items[i]?.id ?? i} style={{ display: i === activeIndex ? 'block' : 'none' }}>
                  {/* Cyan quote mark */}
                  <div style={{ color: '#00E5FF', fontSize: '80px', lineHeight: '0.6', marginBottom: '24px', fontFamily: 'Georgia, serif' }}>&ldquo;</div>
                  <p
                    {...elementProps(sectionId, `items.${i}.quote`, 'text')}
                    style={{
                      fontFamily: "'GeneralSans Variable', 'General Sans', sans-serif",
                  ...
                  <div style={{ marginTop: '24px' }}>
                    ...
                    <p
                      {...elementProps(sectionId, `items.${i}.role`, 'text')}
                      style={{ fontSize: '14px', fontWeight: 400, color: '#FF006E', marginTop: '4px' }}
                    >
                      {authors[i]?.role}
                    </p>
                  </div>
            ...
            {total > 1 && (
              <div {...elementProps(sectionId, 'navDots', 'container', 'Nav Dots')} style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: '8px' }}>
                ...
                <div style={{ marginLeft: 'auto', display: 'flex', gap: '8px' }}>
                  <button
                    onClick={goPrev}
                    style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'transparent', border: '1px solid rgba(0, 229, 255, 0.4)', color: '#00E5FF', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'all 0.3s ease' }}
                  >
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6" /></svg>
                  </button>
                  <button
                    onClick={goNext}
                    style={{ width: '36px', height: '36px', borderRadius: '50%', background: '#00E5FF', border: '1px solid #00E5FF', color: '#0D0D0D', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'all 0.3s ease' }}
                  >
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m9 18 6-6-6-6" /></svg>
                  </button>
                </div>
        ...
        {/* RIGHT — Event/Performance Photo (50%) */}
        <div {...elementProps(sectionId, 'imagePanel', 'container', 'Image Panel')} className="relative overflow-hidden flex items-center justify-center" style={{ flex: '1 1 50%', backgroundColor: '#0D0D0D' }}>
          {Array.from({ length: total }).map((_, i) => {
            const item = items[i]
            const src = item?.avatar || avatars[i]
            return (
              <div
                key={item?.id ?? i}
                className="absolute inset-0 flex items-center justify-center"
              ...
              >
                ...
                {/* Subtle cyan gradient overlay on the image */}
                <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(135deg, rgba(0,229,255,0.08) 0%, rgba(255,0,110,0.06) 100%)', zIndex: 1 }} />
              </div>
            )
          })}
        </div>
...
// ─────────────────────────────────────────────
// SAVEUR — Traiteur / Private Chef Featured Testimonials (50/50 split)
...
function SaveurFeatured({ content, items, sectionId }: { content: Partial<TestimonialsContent>; items: TestimonialItem[]; accent: string; styleOverrides?: StyleOverrides; sectionId: string }) {
  ...
  const defaultQuotes = [
    'Notre mariage était parfait grâce à eux. 200 convives servis avec une précision et une qualité dignes d\'un étoilé. Chaque invité nous en parle encore.',
    'Pour notre gala annuel de 400 personnes, ils ont relevé le défi avec brio. Un menu d\'exception et un service sans faille.',
    'Un dîner à domicile inoubliable. Le chef a transformé notre cuisine en restaurant gastronomique le temps d\'une soirée magique.',
  ]

  const defaultAuthors = [
    { name: 'Claire & Julien M.', role: 'Mariage' },
    { name: 'Philippe D.', role: 'Directeur Événementiel' },
    { name: 'Nathalie R.', role: 'Dîner Privé' },
  ]
  ...
  return (
    <section {...elementProps(sectionId, 'wrapper', 'container', 'Testimonials Section')} style={{ backgroundColor: '#1C1917', color: '#FFFFFF', fontFamily: "'GeneralSans Variable', 'General Sans', sans-serif" }}>
      <style>{`
        @media (max-width: 768px) {
          .saveur-testimonial-split { flex-direction: column !important; }
          .saveur-testimonial-split > div { flex: 1 1 100% !important; min-height: 50vh; }
        }
      `}</style>
      <div {...elementProps(sectionId, 'splitLayout', 'container', 'Split Layout')} className="saveur-testimonial-split flex flex-row w-full" style={{ minHeight: '620px' }}>
        {/* LEFT — Quote (50%) */}
        <div {...elementProps(sectionId, 'contentPanel', 'container', 'Content Panel')} className="flex flex-col" style={{ flex: '1 1 50%', backgroundColor: '#151210', color: '#FFFFFF', padding: '60px' }}>
          {/* Top label */}
          <p
            {...elementProps(sectionId, 'eyebrow', 'text')}
            style={{ fontSize: '14px', fontWeight: 500, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#C8A97E', marginBottom: 'auto' }}
          >
            {content.eyebrow || content.title || 'Témoignages'}
          </p>
          ...
          <div className="flex flex-col" style={{ maxWidth: '536px', marginTop: 'auto', marginLeft: 'auto' }}>
            ...
            <div style={{ marginTop: 'auto', marginBottom: '40px', marginLeft: 'auto' }}>
              {quotes.map((quote, i) => (
                <div key={items[i]?.id ?? i} style={{ display: i === activeIndex ? 'block' : 'none' }}>
                  {/* Gold quote mark */}
                  <div style={{ color: '#C8A97E', fontSize: '80px', lineHeight: '0.6', marginBottom: '24px', fontFamily: 'Georgia, serif' }}>&ldquo;</div>
                  <p
                    {...elementProps(sectionId, `items.${i}.quote`, 'text')}
                    style={{
                      fontFamily: "'GeneralSans Variable', 'General Sans', sans-serif",
                  ...
                  <div style={{ marginTop: '24px' }}>
                    ...
                    <p
                      {...elementProps(sectionId, `items.${i}.role`, 'text')}
                      style={{ fontSize: '14px', fontWeight: 400, color: 'rgba(200, 169, 126, 0.7)', marginTop: '4px' }}
                    >
                      {authors[i]?.role}
                    </p>
                    {/* Gold stars */}
                    <div style={{ display: 'flex', gap: '4px', marginTop: '12px' }}>
                      {Array.from({ length: 5 }).map((_, s) => (
                        <span key={s} style={{ color: '#C8A97E', fontSize: '14px' }}>&#9733;</span>
                      ))}
                    </div>
                  </div>
                </div>
            ...
            {total > 1 && (
              <div {...elementProps(sectionId, 'navDots', 'container', 'Nav Dots')} style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: '8px' }}>
                {Array.from({ length: total }).map((_, i) => (
                  <div
                    ...
                    style={{
                      ...
                      height: '8px',
                      borderRadius: '4px',
                      backgroundColor: i === activeIndex ? '#C8A97E' : 'rgba(255, 255, 255, 0.2)',
                      cursor: 'pointer',
                      transition: 'all 0.3s ease',
                    }}
                  />
                ...
                <div style={{ marginLeft: 'auto', display: 'flex', gap: '8px' }}>
                  <button
                    onClick={goPrev}
                    style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'transparent', border: '1px solid rgba(200, 169, 126, 0.4)', color: '#C8A97E', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'all 0.3s ease' }}
                  >
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6" /></svg>
                  </button>
                  <button
                    onClick={goNext}
                    style={{ width: '36px', height: '36px', borderRadius: '50%', background: '#C8A97E', border: '1px solid #C8A97E', color: '#1C1917', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'all 0.3s ease' }}
                  >
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m9 18 6-6-6-6" /></svg>
                  </button>
                </div>
        ...
        {/* RIGHT — Large Initial / Photo (50%) */}
        <div {...elementProps(sectionId, 'imagePanel', 'container', 'Initials Panel')} className="relative overflow-hidden flex items-center justify-center" style={{ flex: '1 1 50%', backgroundColor: '#1C1917' }}>
          {Array.from({ length: total }).map((_, i) => {
            const item = items[i]
            const initial = (item?.author || authors[i]?.name || 'S').charAt(0).toUpperCase()
            return (
...
// ─────────────────────────────────────────────
// ASCENT — Business / Life Coach Featured Testimonials (50/50 split)
...
function AscentFeatured({ content, items, sectionId }: { content: Partial<TestimonialsContent>; items: TestimonialItem[]; accent: string; styleOverrides?: StyleOverrides; sectionId: string }) {
  ...
  const defaultQuotes = [
    'En 6 mois de coaching, j\'ai doublé le chiffre d\'affaires de mon entreprise. Mais surtout, j\'ai retrouvé un équilibre de vie que je pensais impossible.',
    'Son approche est unique : à la fois stratégique et profondément humaine. Elle m\'a aidée à prendre confiance pour demander et obtenir ma promotion.',
    'Le séminaire de leadership a transformé la dynamique de notre comité de direction. Les résultats sont visibles au quotidien.',
  ]

  const defaultAuthors = [
    { name: 'Laurent M.', role: 'CEO Startup Tech' },
    { name: 'Sarah K.', role: 'Directrice Marketing' },
    { name: 'Marc D.', role: 'DRH Groupe International' },
  ]
  ...
  return (
    <section {...elementProps(sectionId, 'wrapper', 'container', 'Testimonials Section')} style={{ backgroundColor: '#111827', color: '#FFFFFF', fontFamily: "'GeneralSans Variable', 'General Sans', sans-serif" }}>
      <style>{`
        @media (max-width: 768px) {
          .ascent-testimonial-split { flex-direction: column !important; }
          .ascent-testimonial-split > div { flex: 1 1 100% !important; min-height: 50vh; }
        }
      `}</style>
      <div {...elementProps(sectionId, 'splitLayout', 'container', 'Split Layout')} className="ascent-testimonial-split flex flex-row w-full" style={{ minHeight: '620px' }}>
        {/* LEFT — Quote (50%) */}
        <div {...elementProps(sectionId, 'contentPanel', 'container', 'Content Panel')} className="flex flex-col" style={{ flex: '1 1 50%', backgroundColor: '#0D1520', color: '#FFFFFF', padding: '60px' }}>
          {/* Top label */}
          <p
            {...elementProps(sectionId, 'eyebrow', 'text')}
            style={{ fontSize: '14px', fontWeight: 500, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#E0B870', marginBottom: 'auto' }}
          >
            {content.eyebrow || content.title || 'Témoignages'}
          </p>
          ...
          <div className="flex flex-col" style={{ maxWidth: '536px', marginTop: 'auto', marginLeft: 'auto' }}>
            ...
            <div style={{ marginTop: 'auto', marginBottom: '40px', marginLeft: 'auto' }}>
              {quotes.map((quote, i) => (
                <div key={items[i]?.id ?? i} style={{ display: i === activeIndex ? 'block' : 'none' }}>
                  {/* Gold quote mark */}
                  <div style={{ color: '#E0B870', fontSize: '80px', lineHeight: '0.6', marginBottom: '24px', fontFamily: 'Georgia, serif' }}>&ldquo;</div>
                  <p
                    {...elementProps(sectionId, `items.${i}.quote`, 'text')}
                    style={{
                      fontFamily: "'GeneralSans Variable', 'General Sans', sans-serif",
                  ...
                  <div style={{ marginTop: '24px' }}>
                    ...
                    <p
                      {...elementProps(sectionId, `items.${i}.role`, 'text')}
                      style={{ fontSize: '14px', fontWeight: 400, color: 'rgba(224, 184, 112, 0.7)', marginTop: '4px' }}
                    >
                      {authors[i]?.role}
                    </p>
                    {/* Gold stars */}
                    <div style={{ display: 'flex', gap: '4px', marginTop: '12px' }}>
                      {Array.from({ length: 5 }).map((_, s) => (
                        <span key={s} style={{ color: '#E0B870', fontSize: '14px' }}>&#9733;</span>
                      ))}
                    </div>
                  </div>
                </div>
            ...
            {total > 1 && (
              <div {...elementProps(sectionId, 'navDots', 'container', 'Nav Dots')} style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: '8px' }}>
                {Array.from({ length: total }).map((_, i) => (
                  <div
                    ...
                    style={{
                      ...
                      height: '8px',
                      borderRadius: '4px',
                      backgroundColor: i === activeIndex ? '#E0B870' : 'rgba(255, 255, 255, 0.2)',
                      cursor: 'pointer',
                      transition: 'all 0.3s ease',
                    }}
                  />
                ...
                <div style={{ marginLeft: 'auto', display: 'flex', gap: '8px' }}>
                  <button
                    onClick={goPrev}
                    style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'transparent', border: '1px solid rgba(224, 184, 112, 0.4)', color: '#E0B870', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'all 0.3s ease' }}
                  >
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6" /></svg>
                  </button>
                  <button
                    onClick={goNext}
                    style={{ width: '36px', height: '36px', borderRadius: '50%', background: '#E0B870', border: '1px solid #E0B870', color: '#111827', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'all 0.3s ease' }}
                  >
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m9 18 6-6-6-6" /></svg>
                  </button>
                </div>
        ...
        {/* RIGHT — Large Initial / Photo (50%) */}
        <div {...elementProps(sectionId, 'imagePanel', 'container', 'Initials Panel')} className="relative overflow-hidden flex items-center justify-center" style={{ flex: '1 1 50%', backgroundColor: '#111827' }}>
          {Array.from({ length: total }).map((_, i) => {
            const item = items[i]
            const initial = (item?.author || authors[i]?.name || 'A').charAt(0).toUpperCase()
            return (
...
function ZenithFeatured({ content, items, sectionId }: { content: Partial<TestimonialsContent>; items: TestimonialItem[]; accent: string; styleOverrides?: StyleOverrides; sectionId: string }) {
  ...
  const defaultQuotes = [
    'Le vinyasa ici m\'a transformée. En 6 mois, j\'ai retrouvé une souplesse que je pensais perdue et une sérénité dans mon quotidien qui change tout.',
    'Les cours de pilates sont exceptionnels. Mon mal de dos chronique a disparu et je me sens plus forte que jamais à 55 ans.',
    'L\'ambiance du studio est unique. On sent qu\'on entre dans une bulle de calme dès la porte franchie. Les profs sont bienveillants et à l\'écoute.',
  ]

  const defaultAuthors = [
    { name: 'Marie L.', role: 'Vinyasa Flow' },
    { name: 'Catherine D.', role: 'Pilates Mat' },
    { name: 'Julie R.', role: 'Méditation & Yin' },
  ]
  ...
  return (
    <section {...elementProps(sectionId, 'wrapper', 'container', 'Testimonials Section')} style={{ backgroundColor: '#1A1A1A', color: '#FFFFFF', fontFamily: "'GeneralSans Variable', 'General Sans', sans-serif" }}>
      <style>{`
        @media (max-width: 768px) {
          .zenith-testimonial-split { flex-direction: column !important; }
          .zenith-testimonial-split > div { flex: 1 1 100% !important; min-height: 50vh; }
        }
      `}</style>
      <div {...elementProps(sectionId, 'splitLayout', 'container', 'Split Layout')} className="zenith-testimonial-split flex flex-row w-full" style={{ minHeight: '620px' }}>
        {/* LEFT — Quote (50%) */}
        <div {...elementProps(sectionId, 'contentPanel', 'container', 'Content Panel')} className="flex flex-col" style={{ flex: '1 1 50%', backgroundColor: '#111111', color: '#FFFFFF', padding: '60px' }}>
          {/* Top label */}
          <p
            {...elementProps(sectionId, 'eyebrow', 'text')}
            style={{ fontSize: '14px', fontWeight: 500, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#A8C5A0', marginBottom: 'auto' }}
          >
            {content.eyebrow || content.title || 'Témoignages'}
          </p>
          ...
          <div className="flex flex-col" style={{ maxWidth: '536px', marginTop: 'auto', marginLeft: 'auto' }}>
            ...
            <div style={{ marginTop: 'auto', marginBottom: '40px', marginLeft: 'auto' }}>
              {quotes.map((quote, i) => (
                <div key={items[i]?.id ?? i} style={{ display: i === activeIndex ? 'block' : 'none' }}>
                  {/* Sage green quote mark */}
                  <div style={{ color: '#A8C5A0', fontSize: '80px', lineHeight: '0.6', marginBottom: '24px', fontFamily: 'Georgia, serif' }}>&ldquo;</div>
                  <p
                    {...elementProps(sectionId, `items.${i}.quote`, 'text')}
                    style={{
                      fontFamily: "'GeneralSans Variable', 'General Sans', sans-serif",
                  ...
                  <div style={{ marginTop: '24px' }}>
                    ...
                    <p
                      {...elementProps(sectionId, `items.${i}.role`, 'text')}
                      style={{ fontSize: '14px', fontWeight: 400, color: 'rgba(255,255,255,0.45)', marginTop: '4px' }}
                    >
                      {authors[i]?.role}
                    </p>
                    {/* Sage stars */}
                    <div style={{ display: 'flex', gap: '4px', marginTop: '12px' }}>
                      {Array.from({ length: 5 }).map((_, s) => (
                        <span key={s} style={{ color: '#A8C5A0', fontSize: '14px' }}>&#9733;</span>
                      ))}
                    </div>
                  </div>
                </div>
            ...
            {total > 1 && (
              <div {...elementProps(sectionId, 'navDots', 'container', 'Nav Dots')} style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: '8px' }}>
                {Array.from({ length: total }).map((_, i) => (
                  <div
                    ...
                    style={{
                      ...
                      height: '8px',
                      borderRadius: '4px',
                      backgroundColor: i === activeIndex ? '#A8C5A0' : 'rgba(255, 255, 255, 0.2)',
                      cursor: 'pointer',
                      transition: 'all 0.3s ease',
                    }}
                  />
                ...
                <div style={{ marginLeft: 'auto', display: 'flex', gap: '8px' }}>
                  <button
                    onClick={goPrev}
                    style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'transparent', border: '1px solid rgba(168, 197, 160, 0.4)', color: '#A8C5A0', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'all 0.3s ease' }}
                  >
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6" /></svg>
                  </button>
                  <button
                    onClick={goNext}
                    style={{ width: '36px', height: '36px', borderRadius: '50%', background: '#A8C5A0', border: '1px solid #A8C5A0', color: '#1A1A1A', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'all 0.3s ease' }}
                  >
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m9 18 6-6-6-6" /></svg>
                  </button>
                </div>
        ...
        <div {...elementProps(sectionId, 'imagePanel', 'container', 'Initials Panel')} className="relative overflow-hidden flex items-center justify-center" style={{ flex: '1 1 50%', backgroundColor: '#1A1A1A' }}>
          {Array.from({ length: total }).map((_, i) => {
            const item = items[i]
            const initial = (item?.author || authors[i]?.name || 'Z').charAt(0).toUpperCase()
            return (
              <div
                key={item?.id ?? i}
                className="absolute inset-0 flex items-center justify-center"
