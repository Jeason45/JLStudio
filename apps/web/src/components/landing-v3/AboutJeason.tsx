...
export const DEFAULT_ABOUT: AboutContent = {
  ...
  lead: "Quand vous m'écrivez, c'est moi qui vous lis. Et qui dessine, code, et livre votre projet. Pas de relais, pas d'attente.",
  paragraphs: [
    ...
    "Mon engagement : à la livraison, vous avez toutes les clés en main. Mettre à jour votre contenu, gérer vos contacts, faire évoluer votre site au fil du temps — sans avoir à me solliciter pour chaque détail.",
  ],
  stats: [
...
interface AboutJeasonProps { content?: AboutContent }

export default function AboutJeason({ content = DEFAULT_ABOUT }: AboutJeasonProps = {}) {
  const stats = content.stats && content.stats.length > 0 ? content.stats : DEFAULT_ABOUT.stats;
  const paragraphs = content.paragraphs && content.paragraphs.length > 0 ? content.paragraphs : DEFAULT_ABOUT.paragraphs;
        ...
        <div className={`relative overflow-hidden ${isMobile ? 'h-[45vh] min-h-[280px]' : 'h-auto'}`}>
          <div ref={imageRef} className="absolute inset-0">
            <div ref={innerImageRef} className="absolute inset-[-15%] will-change-transform">
              {content.profileImage.startsWith('/') ? (
                <Image
        ...
        <div className={`flex items-center ${isMobile ? 'px-5 py-12' : 'px-12 lg:px-20 py-20'}`}>
          <div ref={contentRef} className="w-full max-w-xl mx-auto sm:mx-0">
            ...
            <div className="about-fade opacity-0 mb-5 sm:mb-7">
              ...
              >
                <span className="text-[#638BFF]">{content.titleLine2}</span>.
              </SplitTextReveal>
            </div>
            ...
            {paragraphs.map((para, i) => (
              <p
                key={i}
                className={`about-fade text-sm sm:text-base text-white/60 leading-relaxed opacity-0 ${i === paragraphs.length - 1 ? 'mb-8 sm:mb-10' : 'mb-4 sm:mb-5'}`}
              >
                {para}
              </p>
            ))}
