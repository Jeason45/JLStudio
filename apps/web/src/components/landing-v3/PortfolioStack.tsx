...
/* ── Fan config ── */
const FAN_ROTATIONS = [-16, -8, 0, 8, 16]; // degrees
const FAN_Y_OFFSETS = [30, 10, 0, 10, 30]; // px — arc shape
const FAN_X_SPREAD = 220; // px spacing between cards in fan
...
export default function PortfolioStack() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // ── Initial: all cards stacked in center as a messy pile ──
      ...
      // ── Phase 1: Cards appear as a pile (quick stagger) ──
      tl.to(cards, {
        opacity: 1,
        scale: 0.85,
        stagger: 0.08,
      ...
      // ── Phase 2: Pile shuffles slightly ──
      tl.to({}, { duration: 0.5 });

      // ── Phase 3: Fan opens — cards spread into arc ──
      const mid = Math.floor(projects.length / 2);
      cards.forEach((card, i) => {
        const xOffset = (i - mid) * FAN_X_SPREAD;
        tl.to(card, {
          x: xOffset,
          y: FAN_Y_OFFSETS[i],
      ...
      if (titleEl) {
        ...
        gsap.set(split.chars, { opacity: 0, y: 60, rotateX: -80 });
        tl.to(split.chars, {
      ...
      // Mark fan as open at end of animation
      tl.call(() => setFanOpen(true));
  ...
  // ── Hover handlers (GSAP for smooth animation) ──
  const handleCardHover = useCallback((index: number) => {
    if (!fanOpen || activeCard !== null || !sectionRef.current) return;
    ...
    cards.forEach((card, i) => {
      if (i === index) {
        gsap.to(card, {
          y: FAN_Y_OFFSETS[i] - 60,
          scale: 1.08,
          rotation: FAN_ROTATIONS[i] * 0.3, // flatten rotation
          zIndex: 20,
          duration: 0.4,
          ease: 'power3.out',
        });
      } else {
        // Push siblings away slightly
        const dir = i < index ? -1 : 1;
        gsap.to(card, {
          x: (i - mid) * FAN_X_SPREAD + dir * 15,
          scale: 0.95,
          opacity: 0.5,
          duration: 0.4,
          ease: 'power2.out',
  ...
  /* ═══════════════════════════════════════════
     MOBILE: simple vertical scroll layout
     ═══════════════════════════════════════════ */
  if (isMobile) {
    return (
      <section id="projets" className="relative bg-[#0a0a0a] py-16 px-4">
        ...
        <div className="space-y-6 max-w-lg mx-auto">
          {projects.map((project, i) => (
            <div
              key={i}
              className="rounded-2xl border border-white/[0.08] bg-[#111114] overflow-hidden"
            >
              ...
              <div className="p-5">
                <p className="text-[#638BFF]/70 text-xs font-medium tracking-[0.3em] uppercase mb-2">
                  {project.category}
                </p>
                <h3 className="font-[family-name:var(--font-outfit)] font-black text-white text-xl mb-3">
                  {project.title}
                </h3>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {project.tags.map((tag, j) => (
                    <span key={j} className="text-[10px] text-white/50 border border-white/[0.1] bg-white/[0.03] px-2 py-0.5 rounded-full">
                      {tag}
                    </span>
                  ))}
                </div>
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: 'radial-gradient(ellipse, rgba(99,139,255,0.06) 0%, transparent 70%)',
        }}
      />

      {/* Section title */}
      <div className="absolute top-0 left-0 right-0 z-[5] pt-8 text-center pointer-events-none">
        <p className="text-[#638BFF]/70 text-xs tracking-[0.4em] uppercase mb-3">Portfolio</p>
        <h2
          data-section-title
      ...
      {/* Hint text */}
      <div
        className="absolute bottom-8 left-1/2 -translate-x-1/2 z-[5] pointer-events-none transition-opacity duration-500"
        style={{ opacity: fanOpen && activeCard === null ? 0.5 : 0 }}
      >
        <p className="text-white/40 text-xs tracking-[0.2em] uppercase flex items-center gap-2">
